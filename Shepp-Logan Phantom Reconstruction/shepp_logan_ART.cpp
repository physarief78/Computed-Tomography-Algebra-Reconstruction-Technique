...
// Radon transform with OpenMP parallelization
    ...
    for (size_t i = 0; i < angles.size(); i++) {
        ...
        for (int t = 0; t < size; t++) {
            ...
            for (int s = 0; s < size; s++) {
                int x = static_cast<int>(center + (t - center) * cos(theta) - (s - center) * sin(theta));
                int y = static_cast<int>(center + (t - center) * sin(theta) + (s - center) * cos(theta));
...
// Compute the TV gradient with OpenMP parallelization
...
// ART with TV regularization, parallelized using OpenMP
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        for (size_t i = 0; i < angles.size(); i++) {
            ...
            for (int t = 0; t < size; t++) {
                ...
                double correction = (sinogram[i][t] - sum_val) / size;
                for (int s = 0; s < size; s++) {
                    int x = static_cast<int>(center + (t - center) * cos(theta) - (s - center) * sin(theta));
                    int y = static_cast<int>(center + (t - center) * sin(theta) + (s - center) * cos(theta));
                    if (x >= 0 && x < size && y >= 0 && y < size) {
